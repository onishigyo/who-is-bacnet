...
export const steps: StepContent[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'ですが、今度は入り口で止まります。持ち込まれた PC は証明書を持っていません。実験で試したところ、TCP・TLS のやり取り自体はひとまず始まりますが、直後にごくわずかなやり取りをしただけで接続が切られました。正規の機器どうしの接続が 24 秒間使われ続けたのに対し、この接続はわずか 0.05 秒で終わっています。IP 編では「同じネットワークにいれば会話に割り込めた」のが、SC では「繋がったように見えても、会話が始まる前に切られる」に変わります。',
    ...
    notes: [
      {
        id: 'sc-std-cert-gate',
        confidence: 'standard',
        text: 'BACnet/SC は、Annex AB.7.4 で BACnet/SC の WebSocket 接続に相互 TLS 認証（mutual TLS）を要求しています。各機器は X.509 証明書で自分を証明し、ハブもそれを検証します。証明書を示せない、あるいは正しい発行者のものでなければ、ハブとの間で BACnet/SC の会話（Who-Is や ReadProperty）が成立する段階まで進めません。',
        source: 'ANSI/ASHRAE Standard 135-2020 Annex AB.7.4（BACnet/SC）',
      },
      {
        id: 'sc-std-encrypted',
        confidence: 'standard',
        text: 'TLS 1.3 では、ハンドシェイクの途中（Server Hello の後）からやり取りが暗号化されます。証明書の交換もこの暗号化領域に入るため、傍受者は証明書の中身すら読めません。同じ理由で、ハンドシェイク完了後に送られる Alert（エラー通知）も暗号化され、復号鍵を持たない Wireshark からは通常の Application Data と見分けがつきません。「Wireshark で読めない」ことは弱点ではなく、盗み見を防ぐという目的の達成そのものです。',
        source:
          'RFC 8446（TLS 1.3）/ 実験キャプチャ（sc_encrypted_traffic・sc_rejected_traffic）',
      },
      {
        id: 'sc-interp-rejection',
        confidence: 'interpretation',
        text: '制作者の実験では、証明書のないノードからの接続は、TCP・TLS のハンドシェイクらしきやり取りをした直後、わずか 0.05 秒で切断されました（正規の接続は 24 秒間使われ続けています）。この極端な短さは拒否があったことを強く示唆しますが、TLS 1.3 は暗号化後の Alert を Application Data と区別できない形にするため、「証明書エラーで拒否された」と Wireshark だけから断定はできません。拒否の理由（CertificateRequired / UnknownIssuer など）は、ハブ側のログで確認でき次第、ここを確立事実に更新します。',
      },
    ],
  },
  {
