...
export const steps: StepContent[] = [
  ...
  },
  {
    ...
    paragraphs: [
      'まず、既存の機器（図の下半分）。SC に対応していない電力計はハブに参加できず、旧来の BACnet/IP の区画に残ります。そこに PC を持ち込まれれば、IP 編と同じく操作できてしまいます。しかもルータで通信を絞っていなければ、その要求は SC 側の機器にも届きます（ルータへの赤い線）。',
      '次に、運用（図の右上）。照明コントローラは証明書の期限が切れていて、ハブに繋がれません。証明書は持っているだけでは守れず、正しく発行し、期限を管理して、はじめて役に立ちます。',
