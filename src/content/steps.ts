...
export const steps: StepContent[] = [
  ...
  },
  {
    ...
    paragraphs: [
      '平和だったネットワークに、持ち込まれた PC が 1 台つながりました。図の下の「持ち込まれた PC を操作する」を押すと、その PC からの通信が流れます。ステップ3とまったく同じ操作です。',
  ...
  },
  {
    ...
    paragraphs: [
      'ここまで見てきた無認証・丸見えの問題に、規格の側から答えたのが BACnet/SC（Secure Connect）です。図が変わりました。真ん中の「ハブ」に、機器がそれぞれ繋ぎにいきます。ここではハブを専用の箱としてではなく、中央監視装置がその役割（ハブ機能）を兼ねる構成にしています。',
      ...
      '下の「会話を始める」を押すと、機器がハブに接続し、これまでと同じ室温の読み書きをする様子が流れます。ただし中身は暗号化されていて、傍受しても読めません ── それは次のステップの答え合わせで確かめます。',
    ],
    notes: [
      {
        id: 'sc-std-topology',
      ...
      },
      {
        ...
        confidence: 'standard',
        text: 'ハブは専用の箱とは限りません。「ハブ機能（hub function）」という役割で、単純な機器から施設全体のワークステーションまで、どのノードでも担えます。ハブは単一障害点になるため、規格はプライマリのハブとフェイルオーバー用のハブの両方に繋げることをノードに求めています。この図では、中央監視装置がハブ機能を兼ねる構成にしています（専用のハブ機器を置く構成もあります）。',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'ですが、今度は入り口で止まります。ハブは接続してきた相手に証明書を求め、持ち込まれた PC はそれを出せません。TLS のハンドシェイクは完了せず、接続そのものが成立しない。IP 編では「同じネットワークにいれば会話に割り込めた」のが、SC では「証明書がなければ会話に入ることすらできない」に変わります。',
      'そして、仮にネットワークを傍受できても、正規の機器どうしの通信は TLS で暗号化されています。Wireshark で見ても中身は Application Data としか映らず、何を読み書きしているのかは分かりません。',
      '下のコンソール…ではなく「持ち込まれた PC を操作する」を押すと、ハブへの接続が拒否される様子が流れます。そのあと、IP と SC の Wireshark を並べた答え合わせで、「読める」と「読めない」の違いを見比べてください。',
    ],
    notes: [
      {
        id: 'sc-std-cert-gate',
        confidence: 'standard',
        text: 'BACnet/SC では、ネットワークに参加できるかどうかを証明書が決めます。有効な証明書を示せないノードは、TLS のハンドシェイク段階で拒否され、アプリケーション層の会話（Who-Is や ReadProperty）にたどり着けません。IP 編のように「ネットワークに到達できれば要求が処理される」構造ではなくなります。',
        source: 'ANSI/ASHRAE Standard 135（Addendum 135-2016bj）',
      },
      {
        id: 'sc-std-encrypted',
        confidence: 'standard',
        text: 'TLS 1.3 では、ハンドシェイクの途中（Server Hello の後）からやり取りが暗号化されます。証明書の交換もこの暗号化領域に入るため、傍受者は証明書の中身すら読めません。「Wireshark で読めない」ことは弱点ではなく、盗み見を防ぐという目的の達成そのものです。',
        source: 'RFC 8446（TLS 1.3）/ 実験キャプチャ（sc_encrypted_traffic）',
      },
      {
        id: 'sc-interp-rejection',
        confidence: 'interpretation',
        text: '証明書のないノードが拒否されるとき、ハブがどんなエラーを返すか（CertificateRequired / UnknownIssuer など）は、実装や設定によって異なります。制作者の実験で観測した具体的な挙動は、答え合わせの素材が揃い次第ここに反映します。現時点では「拒否される」ことまでを確立事実として扱います。',
      },
    ],
  },
  {
