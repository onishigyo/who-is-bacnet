...
const SC_PROVENANCE_BASE =
  CAPTURED_BY +
  '192.168.222.130 で SC ハブを動かし、192.168.222.128 から証明書を持たせずに繋ぎました。「証明書あり」の数字（1157 バイト）は、同じ環境で証明書を持たせて繋いだ別の記録の値です。ほかの機器は物語上の存在で、実験にはいません。'
...
export const scRejectedCapture: CaptureEvidence = {
  ...
  caption:
    'TLS の始まり（271, 276）までは、証明書ありのときと同じです。違うのは 278：証明書ありなら 1157 バイトあるデータが 77 バイトしかありません。ハブは 19 バイト返しただけで（279）、接続は終わりました（280-282）。',
