...
export const scRejectedCapture: CaptureEvidence = {
  ...
  caption:
    'TCP（265-267）と TLS の始まり（271, 276）までは、証明書ありのときと同じです。違いは 278：証明書ありでは 1157 バイトあった暗号化データが、77 バイトしかありません。証明書が入っていない大きさです。ハブの返事は 19 バイトだけ（279）。PC は気づかずに続きを送りますが（280）、返事はなく、接続はそのまま終わりました（281, 282）。',
  filter: 'tcp.port==47900',
  provenance:
    SC_PROVENANCE_BASE +
    'この記録では、192.168.222.128 に証明書を持たせずに接続しています。比べている「証明書あり」の数字（1157 バイト・221 バイト）は、同じ環境で証明書を持たせて繋いだ別の記録から取りました。',
