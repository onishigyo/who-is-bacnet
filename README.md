...
ステップ4の答え合わせは、制作者が閉域の実験環境で取得したキャプチャです。`src/content/captures.ts` に、tshark で表示フィルタをかけて出力した行を**そのまま**持っています（Info 欄の空白の数も含めて手を加えない）。

- BACnet/IP: 表示フィルタ `bacnet`
- BACnet/SC: 表示フィルタ `tcp.port==47900`（SC 編で使う）
